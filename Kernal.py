...
BOT_VER = {'rev': 10, 'botver': {'name': 'STORM', 'ver': 'ver. 5.2.3-%s ', 'os': 'ST M5 AL' }}
################################################################################
...
def call_command_handlers(command, type, source, parameters, callee):
	...
	if COMMAND_HANDLERS.has_key(command):
		if has_access(source, real_access, source[1]):
			...
			reply(type, source, u'أسـف أنت تحتـاج الى اكسز :-)')
		...
		if fromjid.getStripped() in COMMOFF and command in COMMOFF[fromjid.getStripped()]:
			...
			if fromjid.getStripped() in GROUPCHATS:		
				if GCHCFGS[fromjid.getStripped()]['autoaway']==1:
					...
					LAST['gch'][fromjid.getStripped()]['thr']=threading.Timer(600,change_bot_status,(fromjid.getStripped(), u'انا لا افعل شئ منذ \n'+time.strftime('%d.%m.%Y@%H:%M:%S GMT', time.gmtime()), 'away',1))
			...
			with smph:
				...
				if groupchat in LAST['gch'].keys():
					if GCHCFGS[groupchat]['autoaway']==1:
						LAST['gch'][groupchat]['thr']=threading.Timer(600,change_bot_status,(groupchat, u'انا لا افعل شئ منذ \n'+time.strftime('%d.%m.%Y@%H:%M:%S GMT', time.gmtime()), 'away',1))

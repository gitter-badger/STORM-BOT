...
#*******************************************
	...
	if parameters != '':
		...
		if (len(command) == 1) & (command[0] == u'option'):
			reply(type, source, u'تخصيص قائمة عرض المعلومات في قيادة المؤتمرات الخيار،: \ ن * رقم \ ن * أسيوط \ ن * botnick \ ن * ALLUSERS \ ن * redusers \ ن * yellusers \ ن * botstatus \ ن * botadmin \ ن ** shortserver \ ن ** servdef \ ن *** تجد - البحث عن مجموعات من الحروف في مؤتمر')
			return
		...
		if (len(command) == 3) & (command[0] == u'option'):
			if not command[1] in allkeys:
				reply(type, source, u'ادخال خاطئ')
				return
			...
			if command[1] == u'number':
				if (command[2] == '1') | (command[2] == '0'):
					...
					if command[2] == '0':
						res = u'disabled'
					reply(type, source, u'عرض عدد '+res)
			if command[1] == u'conf':
				if (command[2] == '1') | (command[2] == '0'):
					set_conf = command[2]
			...
			if command[1] == u'shortserver':
				if (command[2] == '1') | (command[2] == '0'):
					...
					if command[2] == '0':
						res = u'disabled'
					reply(type, source, u'استخدام الخادم قصير '+res)
			if command[1] == u'serverdef':
				if (command[2].count('.') > 0):
					...
					write_file(DBPATH, str(setup))
					res = command[2]
					reply(type, source, u'تعيين الملقم الافتراضي: '+res)
			return
#********************************************************
...
#********************************************************
	...
	out = u'انا فى هذه الرومات: ('+str(len(GROUPCHATS.keys()))+u'):\n'+commhelp+'\n'
...
def admin_searchman(type, source, parameters):
	...
	if confs == '':
		...
		reply(type,source,u'تم الأرسال في الخاص')
	else:
		aa = u'The user found at:\n[#][conf][(nick)][jid]\n'+ confs
		reply(type,source,u'sent to private')
	reply('أنظر في الخاص', source, aa)
...
def admin_getglobadmin(type, source, parameters):
        res = u'قائمه الاكيز الكلى:\n[#][jid][access]'
        i = 0
        if len(GLOBACCESS) == 1:
                reply(type, source, u'لم اجد الاكسز الكلى')
                return
...
def admin_getlocaladminthisconf(type, source, parameters):
        ...
        if not ACCBYCONF.has_key(source[1]):
                reply(type, source, u'لاتوجد قائمه اكسز كلى فى هذه الروم')
                return
...
def admin_getlocaladminallconf(type, source, parameters):
        res = u'الاكسز الكلى فى جميع الرومات  هو ع الشكل التالي('+str(len(GROUPCHATS))+u'):\n[#][conf][jid][access]'
        i = 0
        if len(ACCBYCONF) == 0:
                reply(type, source, u'لم اجد الاكسز الكلى')
                return
...
def admin_setmsglim(type, source, parameters):
        try:
                ...
                reply(type, source, u'لا خيار يقين!')
                return
...
#register_command_handler(greet_global_add, 'تحيه_عامه', ['admin','new','all'], 40, ' Add global greeting (highest priority)', 'globgreet <nick/jid>=<greeting>', ['globgreet Foo=Hello, how are you %NICK%?'])
#register_command_handler(greet_global_del, 'مسح_تحيه_عامه', ['admin','new','all'], 40, 'Remove global greeting', 'globgreetdel <nick/jid>', ['globgreetdel Foo'])
#register_command_handler(greet_global_show, 'قايمه_تحيه_عامه', ['admin','new','all'], 20, 'Display all welcome', 'globgreetshow', ['globgreetshow'])
#register_command_handler(greet_global_info, 'معلومات_تحيه_عامه', ['admin','new','all'], 10, 'Show info about the plugin', 'globgreetinfo', ['globgreetinfo'])


#register_command_handler(timer_common, 'المؤقت', ['admin','all', 'new'], 20, '!!!Do not work so far. Work proceeds!!! Sets the timer for the expiration of which will be performed by one or other command', 'timer <command> <timeout in seconds> [parameters]', ['timer say 3600 It has been one more hour', 'timer ping 1800'])
#register_command_handler(timer_start, 'بدء_المؤقت', ['superadmin','admin','all', 'new'], 100, '!!!Do not work so far. Work proceeds!!! Start all timers', 'timer_start', ['timer_start'])
#register_command_handler(timer_stop, 'ايقاف_المؤقت', ['admin','all', 'new'], 20, '!!!Do not work so far. Work proceeds!!! Stop timer on PIDu reference given in the appointment of command! Срабатывает вконце цикла!!!! ', 'timer_stop <PID>', ['timer_stop 456184'])
#register_command_handler(timer_show, 'المؤقتات', ['admin','all', 'new'], 20, '!!!Do not work so far. Work proceeds!!! See all timers for this conference', 'timers', ['timers'])
register_command_handler(admin_getglobadmin, 'اكسز_كلى', ['superadmin','admin','all', 'new'], 100, 'Display all global access', 'glob_access', ['glob_access'])
register_command_handler(admin_getlocaladminthisconf, 'الغاء_اكسز', ['admin','all', 'new'], 20, 'Extended command "washere".\nDisplay local access this conference', 'local_access', ['local_access'])
register_command_handler(admin_getlocaladminallconf, 'اكسز_محلى_كلى', ['superadmin','admin','all', 'new'], 100, 'Show local access to all conferences. WARNING! Huge list could be listed', 'local_access_all', ['local_access_all'])
#register_command_handler(admin_getrecentusers, 'كان_هنا', ['admin','all', 'new'], 10, 'Display list of users which ever been saw by bot in a conference.\nManagement:\n1. xwhowas - withdrawal of all who had join TODAY\n2. xwhowas jid on/off - on/off display JID\n3. xwhowas show all - Display the entire log attending conferences! The title is written during. !!!WARNING!!! It could be a huge log!\n4. xwhowas find <sequence of letters> - search jids and nicks log this sequence. If the JID is hidden, info about the match in the JID will not be published.', 'xwhowas [parameter1] [parameter2]', ['xwhowas', 'xwhowas show all', 'xwhowas jid off'])

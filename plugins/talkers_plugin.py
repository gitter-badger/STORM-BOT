...
def talkers_clear(type, source, body):
        ...
        if user_level(room+'/'+source[2], room)>=20:
                if not T.has_key(room):
                        ...
                        reply(type, source, u'تهيئة لحذف قاعدة البيانات ('+str(len(TALKERS_BD[room]))+u' entries), تشغيل الأمر مرة أخرى لإزالته!')
                        return
                else:
                        if time.time() - T[room]['time'] <= 60:
                                if jid == T[room]['jid']:
                                        ...
                                        reply(type, source, u'يتم مسح قاعدة البيانات، ويتم تسجيل العمل في السجل')
                                        return
                                else:
                                        reply(type, source, u'وينبغي تأكيد أن تكون هي نفسها التي تهيئة إزالة!')
                                        return
                        else:
                                reply(type, source, u'مهلة 60 ثانية!')
                                del T[room]
                                return
        else:
                reply(type, source, u'الإنسان قليل!')
                return
...
def talkers_get(type, source, parameters):
        ...
        if not parameters:
                if TALKERS_BD[room].has_key(jid.lower()):
                        ...
                        reply(type, source, u'يبدو وكأنه لا يوجد لديك إحصاءات')
        else:
                if parameters in ['info', 'clear', 'top']:
                        if parameters == 'top':
                                ...
                                reply(type, source, u'المعلمة التي تم تحديدها على أنها أمر داخلي، ولكن ليس الآن')
                        ...
                        if TALKERS_BD[room].has_key( get_true_jid(room+'/'+parameters).lower() ):
                                ...
                                reply(type, source, u'مثله لا توجد إحصاءات')
                        ...
                        reply(type, source, u':-P، لم يتم التعرف على المعلمة')

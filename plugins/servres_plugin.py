...
def handler_check_servstat(type, source, body):
	...
	reply(type, source, u'مجموع الخوادم '+str(col)+' :'+list)
...
def handler_check_resstat(type, source, body):
	...
	for item in stat:
		...
		if col <= 20:
			list += '\n'+str(col)+'. '+item[1]+' - '+str(item[0])
	reply(type, source, u'مجموع الموارد '+str(col)+' :'+list)

register_join_handler(handler_servres_stat)
register_command_handler(handler_check_servstat, 'الخادم', ['все','инфо'], 10, 'Выдаёт топ 20 серверов, зафиксированных за рабочую сессию, с которых юзеры входили в конференции, а также колличество использований каждого\nBy WitcherGeralt\nhttp://witcher-team.ucoz.ru/', 'сервера', ['сервера'])
register_command_handler(handler_check_resstat, 'موارد', ['все','инфо'], 10, 'Выдаёт топ 20 ресурсов, зафиксированных за рабочую сессию, с которыми юзеры входили в конференции, а также колличество использований каждого\nBy WitcherGeralt\nhttp://witcher-team.ucoz.ru/', 'ресурсы', ['ресурсы'])

...
try:
	import json
	register_command_handler(handler_google_trans, 'ترجم', ['info','all'], 10, 'Translate from one language to another. Via Google Translate engine. Available languages for translation:\n' + ', '.join(sorted([x.encode('utf-8')+': '+y.encode('utf-8') for x,y in trans_langs.iteritems()])), 'trans <source_lang> <target_lang> <text>', ['trans en ru hello', 'trans ru en привет'])
except ImportError:
	try:

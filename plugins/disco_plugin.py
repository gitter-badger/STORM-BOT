	...
	if res:
		if res.getType() == 'result':
			...
			if disco:
				...
				reply(type, source, u'هذه الروم فارغة ;-)')
			return
		else:
			rep = u'أسف لا أستطيع يوجد خطأ'
	else:
		rep = u'اسف...'
...
def handler_disco_answ(type,source,stop,disco,srch):
	...
	if total==stop:
		...
		return
	rep,dis,disco = u'أرى في هذه الروم:\n',[],sortdis(disco)
	for item in disco:
		if len(item)==3:
			total+=1
			if srch:
	...
	if dis:
		if len(disco)!=len(dis):
			dis.append(u'المجموع '+str(len(disco))+u' users')
	else:
		rep=u'هذه الروم فارغة ;-)'

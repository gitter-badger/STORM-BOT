...
def handler_python_calc(type, source, parameters):
	...
	if re.sub('([' + string.digits +']|[\+\-\/\*\^\.])','',parameters).strip() == '':
		...
		return_value = u'علمنى كيف اعملهايلا ولو'
	else:
		return_value = u'لديك خلل فى النظام ديربالك ها '
	reply(type, source, return_value)

register_command_handler(handler_python_eval, 'اظهار-تعبير-بايثون', ['superadmin','all'], 200, 'Executes and shows the set of python expression.', 'eval <expression>', ['eval 1+1'])
register_command_handler(handler_python_exec, 'تعبير-بايثون', ['superadmin','all'], 200, 'Executes expression of python.', 'exec <expression>', ['eval pass'])
register_command_handler(handler_python_sh, 'sh', ['superadmin','all'], 200, 'Executes shell command.', 'sh <command>', ['sh ls'])
register_command_handler(handler_python_calc, 'حساب', ['info','all'], 10, 'Calculator.', 'calc <expression>', ['calc 1+2'])

...
#     print GROUPCHATS.keys()
     for groupchat in lgcts: #GROUPCHATS:
       leave_groupchat(groupchat, u'اعاده ارسال كليه من اونر الروم 8-)')
...
register_command_handler(handler_rejoinall, 'اعاده-ارسال-كليه', ['admin','muc','all'], 100, 'Rejoin bot to conferences according to database.', 'rejoinall', ['rejoinall'])

register_command_handler(handler_changestatus, 'حاله-البوت', ['muc','admin','all'],100, 'Change bot status in the current conference, if two parameters are not mentioned, bots will use the default status.', 'setstatus[online|chat|away|xa|dnd] [message]', ['setstatus chat','setstatus dnd','setstatus away meeting!!','setstatus'])
